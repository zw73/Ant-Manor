...
module.exports = {
  ...
  clickPointRandom: function (x, y) {
    x = Math.round(randomRange(x-5, x+5))
    y = Math.round(randomRange(y-5, y+5))
  ...
  clickRandom: function (obj) {
    ...
    let x = Math.round(randomRange(left, right))
    let y = Math.round(randomRange(top, bottom))
  ...
  clickRandomRegion: function (region) {
    ...
    let x = Math.round(randomRange(left, left + width))
    let y = Math.round(randomRange(top, top + height))
...
function Automation () {
  ...
  this.gesturePath = function (start, end, duration) {
    ...
    //设置随机滑动时长范围
    var timeMin = duration? duration-500 : 1000
    var timeMax = duration? duration+500 : 2000

    var time=[0,random(timeMin,timeMax)]
    var track=generateRandomCubicBezierPath(sx,sy,ex,ey)
    
    let param = time.concat(track)
    myGestures(param)
...
function getRandomControlPoints (sx, sy, ex, ey) {
  ...
  if(Math.abs(ex-sx)>Math.abs(ey-sy)){
      var my=(sy+ey)/2
      controlY1=my+random(0,leaveHeightLength)
      controlY2=my-random(0,leaveHeightLength)
  
      var lx=(sx-ex)/3
      if(lx<0){lx=-lx}
      controlX1=sx+lx/2+random(0,lx)
      controlX2=sx+lx+lx/2+random(0,lx)
  }else{
      var mx=(sx+ex)/2
      controlY1=mx+random(0,leaveHeightLength)
      controlY2=mx-random(0,leaveHeightLength)

      var ly=(sy-ey)/3
      if(ly<0){ly=-ly}
      controlY1=sy+ly/2+random(0,ly)
      controlY2=sx+ly+ly/2+random(0,ly)
  }
...
function generateRandomCubicBezierPath (x1, y1, x2, y2, numPoints) {
  ...
  return bezierCreate(x1, y1, controlX1, controlY1, controlX2, controlY2, x2, y2);
}
