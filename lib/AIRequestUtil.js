...
function getQuestionInfo (AIType, AIKey, questionRoot, source) {
  ...
  logFloaty.pushLog('开始获取题目信息：'+source)
  ...
  switch (source) {
    case 'tbfarm':
      ...
      questionWidget = questionRoot.child(0).child(0)
      // debugInfo(["rootDepth: {}", questionRoot.depth()])
      answerButtons = questionRoot.children().find(depth(questionRoot.depth()+1).className('android.widget.Button'))
      // debugInfo(["answerButtons: {}", JSON.stringify(answerButtons)])
      answerWidgets = questionRoot.children().find(depth(questionRoot.depth()+1).className('android.widget.TextView'))
      // debugInfo(["answerWidgets: {}", JSON.stringify(answerWidgets)])
      if (!answerWidgets || answerWidgets.length == 0) {
        answerWidgets = answerButtons
      }
      break;
    default:
      questionWidget = questionRoot.child(0)
      answerWidgets = questionRoot.child(1).children().find(className('android.widget.TextView'))
      answerButtons = answerWidgets
      break;
  }
  if (!(questionWidget && answerWidgets && answerButtons)) {
    logFloaty.pushErrorLog('题目信息获取失败')
    return
  }
  ...
  if (question && answers.length === 2) {
    logFloaty.pushLog('请求AI接口中...')
    let start = new Date().getTime()
    let response = aiRequest(buildRequest(question, answers), AIType, AIKey)
    ...
    if (result) {
      let answerButton = answerButtons[result.answerIdx]
      let target = { x: answerButton.bounds().left, y: answerButton.bounds().centerY() }
      logFloaty.pushLog('答案位置：' + JSON.stringify(target))
      logFloaty.pushLog('答案解释：' + result.describe)
      return { target: { x: answerButton.bounds().centerX(), y: target.y }, describe: result.describe }
    } else {
      logFloaty.pushErrorLog('接口请求失败')
    }
  } else {
    logFloaty.pushErrorLog('题目信息获取失败')
  }
