...
function taskWorker() {
  ...
  this.checkTaskCanRun = function (taskTitle) {
    ...
    if (this.taskStates[taskTitle].failDayCount >= 3) {
      // 如果任务被禁用，检查是否超过5天未尝试
      let lastTryDate = this.taskStates[taskTitle].lastTryDate;
      if (lastTryDate) {
  ...
  this.doTasks = function (taskInfos) {
    ...
    do {
      ...
      for (let i = 0; i < taskInfos.length; i++) {
        ...
        let entryBtns = null
        let finishCurrentTaskType = false
        while (!finishCurrentTaskType && (entryBtns = widgetUtils.widgetGetAll(taskInfo.btnRegex, 3000))) {
          for (let j=0;j<entryBtns.length;j++) {
            ...
            if (titleObj) {
              ...
              if (titleText in executedTasks) {
                ...
                if (j == entryBtns.length - 1) {
                  finishCurrentTaskType = true
                }
                continue
              }
              // 标记任务已处理
              ...
              let task = null
              for (let k = 0; k < taskInfo.tasks.length; k++) {
                if (titleText.match(taskInfo.tasks[k].titleRegex)) {
                  task = taskInfo.tasks[k]
                  break
                }
              }
              if (!task) {
