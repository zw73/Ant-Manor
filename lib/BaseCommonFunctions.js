...
function CommonFunctions () {
    ...
    try {
      ...
      if (!_config.async_waiting_capture) {
        ...
        if (!screen) {
          ...
          if (requestPermission && captureErrorCount++ < errorLimit) {
  ...
  this.clickBackOrClose = function (package, repeatTime) {
    ...
    if ((backOrColse /*= this._widgetUtils.widgetGetOne('返回|关闭', 500)*/) 
          ...
          if (descView) {
            ...
            if (sliderBtn) {
              ...
              if (sliderText) {
                ...
                automator.gesturePath({x:bound.centerX(),y:bound.centerY()},{x:slider_bound.right+20,y:slider_bound.centerY()})
                sleep(3000);
