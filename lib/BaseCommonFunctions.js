...
let _LogFloaty = singletonRequire('LogFloaty')
...
function CommonFunctions () {
  ...
  this.checkCaptureScreenPermission = function (errorLimit, releaseLock, captureGap, resetPermission) {
    ...
    if (typeof captureGap === 'undefined' || typeof captureGap != 'number') {
    ...
    try {
      ...
      if (!_config.async_waiting_capture) {
        ...
        do {
          ...
          if (!screen) {
            ...
            if (errorLimit > errorCount + 1) {
              ...
              back()
              sleep(1000)
              if (currentRunning != currentPackage()) {
        ...
        if (!screen) {
          ...
          _logUtils.errorInfo(['获取截图失败多次[{}], 可能已经没有了截图权限，且重新申请权限失败重新执行脚本', captureErrorCount], true)
          automator.back()
          if (releaseLock) {
  ...
  this.waitForTBVerify = function () {
    while (textContains("通过验证以确保正常访问").exists()
      || textContains("请按照说明进行验证").exists() || textContains("系统出了点问题").exists() ) {
      _logUtils.debugInfo('发现淘宝验证弹窗，等待处理')
      this.waitForAction(5,'等待处理淘宝验证弹窗')
    }
  }
  
  this.clickBackOrClose = function (package, repeatTime) {
    _logUtils.debugInfo(['尝试返回操作，包名：{} 重试次数：{}', package, repeatTime])
    repeatTime = repeatTime || 0
    ...
    if (getCurrentPkgName.indexOf('launcher')>=0 || getCurrentPkgName.indexOf('home')>=0) {
      _logUtils.debugInfo('检测到已返回桌面，停止返回操作')
      return false
    }
    let backOrColse = null
    if ((backOrColse = this._widgetUtils.widgetGetOne('返回.*|关闭', 1000, false, false ,(m) =>
              m.boundsInside(0, 0, config.device_width, 300))) 
            // && automator.checkCenterClickable(backOrColse)
    ) {
  ...
  this.minimize = function (currentPackageName, confirmCallback) {
    ...
    try {
      let maxRepeat = 10
      let repeatTime = 0
      while (repeatTime++ < maxRepeat && this.clickBackOrClose(currentPackageName,repeatTime)) {
        confirmCallback && confirmCallback()
        sleep(3000)
        this.waitForTBVerify()
      }
      let getCurrentPkgName = currentPackage()
      _logUtils.debugInfo(['最后一次获取包名：{} 对比目标包名：{}', getCurrentPkgName, currentPackageName])
      if (getCurrentPkgName === currentPackageName) {
        if (currentPackageName.indexOf('launcher')>=0 || currentPackageName.indexOf('home')>=0) {
          home()
        } else {
          _logUtils.debugInfo('尚未成功返回，尝试结束当前应用')
          this.killCurrentApp()
        }
      }
    } catch (e) {
  ...
  this.showDialogAndWait = function (checkDismissReason, notCheckUpdate, taskName) {
    ...
    // 启动淘宝验证弹窗检测线程
    this.checkAndSkipTaobaoVerifyDialog()
    ...
    if (skipDialog) {
      // 最小化当前应用
      if (currentPackageName.indexOf(_config.package_name)>=0) {
        this.minimize()
      // } else {
      //   home()
      //   sleep(1000)
   ...
   *  检测淘宝验证弹窗并跳过
  */
  this.checkAndSkipTaobaoVerifyDialog = function () {
    let stopCheck = false
    let running = false
    let that = this
    _logUtils.debugInfo('淘宝验证检测：添加运行状态监听器，结束时释放淘宝验证弹窗检测')
    this.registerOnEngineRemoved(function () {
      stopCheck = true
      _logUtils.debugInfo('淘宝验证检测：脚本执行结束, 正在移除淘宝验证弹窗检测')
      while (running) {
        sleep(500)
      }
      _logUtils.debugInfo('淘宝验证检测：淘宝验证弹窗检测已移除')
    }, 'checkandskiptaobaoverify')
    
    //启动线程监听toast，需要时多按一次返回键
    threads.start(function () {
      events.observeToast()
      // 监控 toast
      events.onToast(function (toast) {
        let text = toast.getText()
        _logUtils.debugInfo(['获取到toast文本：{}', text])
        if ( toast ) {
          if (/.*再.*退出.*/.test(text)) {
            back()
            sleep(500)
    ...
    threads.start(function (){
      const checkTop = _config.device_height * 0.2
      const checkBottom = _config.device_height
      running = true
      _logUtils.debugInfo('淘宝验证检测：启动淘宝验证弹窗检测')
      try {
        while (!stopCheck) {
          sleep(1000)
          let descView = textContains("通过验证以确保正常访问").findOnce()
          if (descView) {
            _logUtils.debugInfo('淘宝验证检测：发现淘宝验证弹窗')
            sleep(1000)
            // 滑动按钮“>>”位置
            let sliderBtn = idContains("nc_1_n1z").findOnce();
            if (sliderBtn) {
              _logUtils.debugInfo('淘宝验证检测：发现验证滑块')
              var bound = sliderBtn.bounds();
              // 滑动边框位置
              let sliderText = text("向右滑动验证").findOnce();
              if (sliderText) {
                _logUtils.debugInfo('淘宝验证检测：发现验证滑动边框')
                var slider_bound = sliderText.bounds();
  
                _logUtils.debugInfo('淘宝验证检测：滑动验证')
                // 通过更复杂的手势验证（向右滑动过程中途停顿）
                let bx = automator.randomRange(bound.left, bound.right)
                let by = automator.randomRange(bound.top, bound.bottom)
                let sx = automator.randomRange(slider_bound.right+50, slider_bound.right+150)
                let sy = automator.randomRange(slider_bound.top, slider_bound.bottom)
                let duration = automator.randomRange(900,1200)
            
                // automator.gesturePath({x:bx,y:by},{x:sx,y:sy},duration)
                sleep(3000);
  
                if (textContains("验证失败").exists()) {
                  _logUtils.debugInfo( "淘宝验证检测：验证失败，尝试返回" )
                  automator.back();
                } else if (textContains("通过验证以确保正常访问").exists()) {
                  _logUtils.debugInfo('淘宝验证检测：未生效，尝试返回')
                  automator.back();
                } else if (textContains("系统出了点问题").exists()) {
                  _logUtils.debugInfo('淘宝验证检测：系统出了点问题，尝试返回')
                  automator.back();
                } else {
                  _logUtils.debugInfo( "淘宝验证检测：自动完成淘宝验证弹窗" )
                }
              } else {
                _logUtils.debugInfo('淘宝验证检测：未找到滑动按钮，重试')
              }
            } else {
              _logUtils.debugInfo('淘宝验证检测：未找到滑动边框，重试')
            }
            continue
          }
          descView = textContains('系统出了点问题').findOnce()
          if (descView) {
            _logUtils.debugInfo('淘宝验证检测：发现系统出了点问题，尝试返回')
            automator.back();
            continue
          }
          descView = textContains('关闭应用').findOnce()
          if (descView) {
            _logUtils.debugInfo('淘宝验证检测：发现关闭应用，尝试点击')
            automator.clickRandom(descView)
            continue
          }
          descView = textMatches('取消|忽略|拒绝|我知道了').boundsInside(0, checkTop, _config.device_width, checkBottom).findOnce()
          if (descView) {
            _logUtils.debugInfo('淘宝验证检测：发现取消或忽略按钮，等待10秒再次检查')
            sleep(10000)
            descView = textMatches('取消|忽略|拒绝|我知道了').boundsInside(0, checkTop, _config.device_width, checkBottom).findOnce()
            if (descView) {
              _logUtils.debugInfo('淘宝验证检测：发现取消、忽略或拒绝等按钮，尝试点击')
              // automator.clickRandom(descView)
              descView.click()
              continue
            }
        ...
        _logUtils.debugInfo('淘宝验证检测：结束淘宝验证弹窗检测')
      } catch (e) {
        _logUtils.errorInfo('淘宝验证检测：淘宝验证弹窗检测执行异常：' + e)
  ...
  this.cancelAllTimedTasks = function (forceMain) {
    ...
    if (array && array.length > 0) {
      ...
      let mainScriptJs = (forceMain || !_config._force_auto_start_script) ? this.getRunningScriptSource(forceMain) : _config._force_auto_start_script
  ...
  this.killCurrentApp = function () {
    if (_config.killAppWithGesture) {
      ...
      this.killAppBySytemSetting()
    }
  }

  this.killAppBySytemSetting = function (package) {
    package = package || currentPackage()
    if (!package) {
      _logUtils.debugInfo('package为空，无法关闭')
      return
    }
    ...
    _logUtils.debugInfo('等待进入设置界面加载')
    let killed = false
    sleep(1000)
    let stop = this._widgetUtils.widgetWaiting('(结束运行)|(强行停止)', null, 3800)
    if (stop) {
      _logUtils.debugInfo('点击结束运行')
      stop = this._widgetUtils.widgetGetOne('(结束运行)|(强行停止)')
      stop.click()
      sleep(2000)
      let confirm = this._widgetUtils.widgetGetOne('(确定)|(强行停止)')
      if (confirm) {
          _logUtils.debugInfo('点击确认结束运行')
        confirm.click()
        sleep(1000)
        killed = true
      }
    } else {
      _logUtils.debugInfo('未能找到结束运行，通过设置关闭'+package+'失败')
    }
    home()
    return killed
  }
  ...
  */
  ...
  /*检查对象是否在屏幕内
  * @param {UIObject} obj - 要检查的对象
  * @return {boolean} true: 在屏幕内，false: 不在屏幕内
  * */
  this.isObjectInScreen = function (obj) {
    if (!obj) {
      return false
    }
    let bound = obj.bounds()
    return bound.left>=0 && bound.left<=device.width && bound.top>=0 && bound.top<=device.height && bound.right<=device.width && bound.bottom<=device.height
  }

  /*
  * 查找任务入口
  * @param titleRegex 任务标题正则
  * @param btnRegex 按钮正则
  * @param callback 回调函数
  * @param errorCallback 错误回调函数
  * @mode 0: 默认，1：支付宝农场模式，2：淘宝农场模式
  * @needVisible 是否需要可见
  * @return true: 找到了，false: 未找到
  */
  this.findAndOpenTaskPage = function (titleRegex, btnRegex, callback, errorCallback,  mode, needVisible) {
    btnRegex =  '^' + (btnRegex || '去完成') + '$'
    mode = mode || 0
    needVisible = (needVisible==null) ? true : needVisible

    let result = false

    _logUtils.debugInfo(['查找任务标题：' + titleRegex + '，按钮：' + btnRegex + ', mode：' + mode])
    if (!this._widgetUtils) {
      this._widgetUtils = singletonRequire('WidgetUtils')
    }
    let titles = this._widgetUtils.widgetGetAll(titleRegex,1000)
    if (titles&& titles.length > 0) {
      for (let i = 0; i < titles.length; i++) {
        let title = titles[i]
        _logUtils.debugInfo(['找到任务标题：' + title.text()])
        ...
        let entryText = null
        if (entryBtn) {
          if (needVisible && !this.isObjectInScreen(entryBtn)) {
            _logUtils.debugInfo('入口按钮不可见，跳过')
            continue
          }
          entryText = entryBtn.text()
        }
        if (entryText) {
          if (new RegExp(btnRegex).test(entryText)) {
            if (needVisible) {
            ...
            sleep(1000)
            result = callback({ enter: true, btnText: entryText })===false ? false : true
            break
          } else {
            callback({ enter: false, btnText: entryText })
          }
        } else {
          _logUtils.debugInfo('未找到入口按钮，跳过')
        }
      }
    } else {
   ...
   */
  this.getTaskEntryBtnByTitle = function (title, mode) {
    mode = mode || 0
    let entryBtn = null
    let findIndex = 2+title.indexInParent();
    switch(mode) {
   ...
   */
  this.getTaskTitleObj = function (entryBtn, mode) {
    mode = mode || 0
    let title = null
    let findIndex = null
    switch(mode) {
   ...
   * @returns {boolean} 如果应用已安装，则返回true；否则返回false
   */
  this.checkAppInstalledByName = function (name) {
    let appName = null
    if (name.indexOf('点淘')>-1){
      appName = '点淘'
    } else if (name.indexOf('一淘')>-1){
      appName = '一淘'
    } else if (name.indexOf('快手')>-1){
      appName = '快手'
    } else if (name.indexOf('抖音极速版')>-1){
      appName = '抖音极速版'
    } else if (name.indexOf('闲鱼')>-1){
      appName = '闲鱼'
    } else if (name.indexOf('饿了么')>-1){
      appName = '饿了么'
    } else if (name.indexOf('菜鸟')>-1){
      appName = '菜鸟'
    } else if (name.indexOf('天猫')>-1){
      appName = '天猫'
    } else if (/淘宝|淘金币/.test(name)){
      appName = '淘宝'
    } else if (/支付宝|蚂蚁新村/.test(name)){
      appName = '支付宝'
    } else if (name.indexOf('七猫')>-1){
      appName = '七猫免费小说'
    } else if (name.indexOf('头条极速')>-1){
      appName = '头条搜索极速版'
    } else if (name.indexOf('微博')>-1){
      appName = '微博'
    }
    let appPackage = null
    if (appName) {
      appPackage = app.getPackageName(appName)
    }
  ...
  /**
   * 等待一段时间，直到满足条件或者超时
   * @param {number} timeout - 等待的时间，单位为秒
   * @param {string} actionDesc - 等待动作的描述，用于在日志中显示
   * @param {function} action - 一个函数，用于判断是否满足条件，如果满足条件，则返回true；否则返回false
   * @returns {boolean} 如果等待时间超时，则返回false；否则返回true
   */
  this.waitForAction = function (timeout, actionDesc, action) {
    let startTime = new Date().getTime();
    let endTime = startTime + timeout * 1000;
    actionDesc = actionDesc || "";
    let currentTime = new Date().getTime();
    _LogFloaty.show()
    while (currentTime < endTime) {
      _LogFloaty.replaceLastLog(
        actionDesc + " 需等待 " + timeout + "s，已等待：" + (currentTime - startTime) / 1000 + "s"
      );
      if (action && action()) {
        return true;
      }
